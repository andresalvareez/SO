...
while [ "$valid" = false ]; do
    ...
    if [[ "$num" =~ ^[0-9]+$ ]]; then
        ...
        # Si el número es mayor que 0 Y menor que 10, entonces el número está en el intervalo
        if [ "$num" -gt 0 ] && [ "$num" -lt 10 ]; then
            echo "El numero esta en el intervalo"
            valid=true
        # Si el número es menor que 0 O mayor que 10, entonces el número está en el intervalo
        elif [ "$num" -lt 0 ] || [ "$num" -gt 10 ]; then
